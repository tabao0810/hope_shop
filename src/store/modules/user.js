...
const mutations = {
  ...
  addCartMutation(state, payload) {
    try {
      ...
      const size = state.userInfo.carts.findIndex(
        (cart) => cart.sizeClothing === payload.sizeClothing
      );
      const sizeShoe = state.userInfo.carts.findIndex(
        (cart) => cart.sizeShoe === payload.sizeShoe
      );
      const color = state.userInfo.carts.findIndex(
        (cart) => cart.color === payload.color
      );
  ...
  addWishListMutation(state, payload) {
    const index = state.userInfo.wishes.findIndex(
      (cart) => cart._id === payload._id
    );
    if (index !== -1) {
  ...
  removeAllCartMutation(state) {
    (state.userInfo.carts = []), (state.userCarts = []);
  },
};
const actions = {
  // Đăng ký tài khoản
  ...
  handleUpAction(context, payload) {
    context.commit("handleUpMutation", payload), context.dispatch("updateUser");
  },
